...
import { supabase, checkError, ActivityLog } from '../lib/supabase';
...
export default function ActivityMap() {
  ...
  const handleSave = async (form: LogForm & { id?: string }) => {
    ...
    if (form.id) {
      await checkError(supabase.from('activity_logs').update(payload).eq('id', form.id), 'Update activity log');
    } else {
      await checkError(supabase.from('activity_logs').insert(payload), 'Create activity log');
    }
    setLogModal(null);
    load();
  };
  ...
  const handleDelete = async () => {
    ...
    await checkError(supabase.from('activity_logs').delete().eq('id', deleteTarget.id), 'Delete activity log');
