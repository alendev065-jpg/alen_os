...
export default function Body() {
  ...
  const load = useCallback(async () => {
    ...
    if (!error && data) {
      if (data.length === 0) {
        const seeded = await checkError(supabase.from('body_metrics').insert(SEED).select(), 'Seed body metrics');
        setMetrics(seeded || []);
      } else {
        setMetrics(data);
      }
  ...
  const handleSave = async (form: MetricForm & { id?: string }) => {
    ...
    if (form.id) {
      await checkError(supabase.from('body_metrics').update(payload).eq('id', form.id), 'Update body metric');
    } else {
      await checkError(supabase.from('body_metrics').insert(payload), 'Create body metric');
    }
    setModal(null);
    load();
  };
  ...
  const handleDelete = async () => {
    ...
    await checkError(supabase.from('body_metrics').delete().eq('id', deleteTarget.id), 'Delete body metric');
