...
import { supabase, checkError, FinanceTransaction, FinanceReceivable } from '../lib/supabase';
...
export default function Finance() {
  ...
  const handleSaveTx = async (form: TxForm & { id?: string }) => {
    ...
    if (form.id) { await checkError(supabase.from('finance_transactions').update(payload).eq('id', form.id), 'Update transaction'); }
    else { await checkError(supabase.from('finance_transactions').insert(payload), 'Create transaction'); }
  ...
  const handleSaveRecv = async (form: RecvForm & { id?: string }) => {
    ...
    if (form.id) { await checkError(supabase.from('finance_receivables').update(payload).eq('id', form.id), 'Update receivable'); }
    else { await checkError(supabase.from('finance_receivables').insert(payload), 'Create receivable'); }
    setRecvModal(null); load();
  };

  const handleDelete = async () => {
    if (!deleteTarget) return;
    if (deleteTarget.type === 'tx') await checkError(supabase.from('finance_transactions').delete().eq('id', deleteTarget.id), 'Delete transaction');
    else await checkError(supabase.from('finance_receivables').delete().eq('id', deleteTarget.id), 'Delete receivable');
