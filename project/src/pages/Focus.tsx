...
export default function Focus() {
  ...
  const load = useCallback(async () => {
    ...
    if (!error && data) {
      if (data.length === 0) {
        const seeded = await checkError(supabase.from('focus_sessions').insert(SEED).select(), 'Seed focus sessions');
        setSessions(seeded || []);
      } else {
        setSessions(data);
      }
  ...
  const handleTimerComplete = async (duration: number, distractions: number, type: string) => {
    ...
    const data = await checkError(supabase.from('focus_sessions').insert(payload).select().maybeSingle(), 'Log focus session from timer');
  ...
  const handleSave = async (form: SessionForm & { id?: string }) => {
    ...
    if (form.id) {
      await checkError(supabase.from('focus_sessions').update(payload).eq('id', form.id), 'Update focus session');
    } else {
      await checkError(supabase.from('focus_sessions').insert(payload), 'Create focus session');
    }
    setModal(null);
    load();
  };
  ...
  const handleDelete = async () => {
    ...
    await checkError(supabase.from('focus_sessions').delete().eq('id', deleteTarget.id), 'Delete focus session');
