...
import { supabase, checkError, JournalEntry } from '../lib/supabase';
...
export default function Journal() {
  ...
  const save = useCallback(async () => {
    ...
    if (entry.id) {
      await checkError(supabase.from('journal_entries').update(payload).eq('id', entry.id), 'Save journal entry');
    } else {
      const entryNumber = allDates.length + 1;
      const data = await checkError(supabase.from('journal_entries').insert({ ...payload, entry_number: entryNumber }).select().single(), 'Create journal entry');
      if (data) {
        setEntry(data);
  ...
  const handleDelete = async () => {
    if (!entry.id) return;
    await checkError(supabase.from('journal_entries').delete().eq('id', entry.id), 'Delete journal entry');
