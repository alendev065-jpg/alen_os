...
import { supabase, CrmPipeline, CrmContact } from '../lib/supabase';
...
export default function Dashboard({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  const handleSaveDeal = async (form: DealForm & { id?: string }) => {
    ...
    if (form.id) { await supabase.from('crm_pipeline').update(payload).eq('id', form.id); }
    else { await supabase.from('crm_pipeline').insert(payload); }
  ...
  const handleSaveContact = async (form: ContactForm & { id?: string }) => {
    ...
    if (form.id) { await supabase.from('crm_contacts').update(payload).eq('id', form.id); }
    else { await supabase.from('crm_contacts').insert(payload); }
    setContactModal(null); load();
  };

  const handleDelete = async () => {
    if (!deleteTarget) return;
    if (deleteTarget.type === 'deal') await supabase.from('crm_pipeline').delete().eq('id', deleteTarget.id);
    else await supabase.from('crm_contacts').delete().eq('id', deleteTarget.id);
