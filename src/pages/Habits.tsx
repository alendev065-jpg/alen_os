...
export default function Habits() {
  ...
  const handleSave = async (form: HabitForm & { id?: string }) => {
    if (form.id) {
      await supabase.from('habits').update({ name: form.name, frequency: form.frequency, status: form.status, streak: form.streak }).eq('id', form.id);
    } else {
      await supabase.from('habits').insert({ name: form.name, frequency: form.frequency, status: form.status, streak: form.streak });
    }
    setModal(null);
    load();
  };
  ...
  const toggleToday = async (habit: Habit) => {
    ...
    await supabase.from('habits').update({ status: newStatus, last_completed: today }).eq('id', habit.id);
    load();
  };
