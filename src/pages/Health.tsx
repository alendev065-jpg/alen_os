...
export default function Health() {
  ...
  const load = useCallback(async () => {
    ...
    if (!error && data) {
      if (data.length === 0) {
        const { data: seeded } = await supabase.from('health_logs').insert(SEED).select();
        setLogs(seeded || []);
      } else {
        setLogs(data);
      }
  ...
  const handleSave = async (form: HealthForm & { id?: string }) => {
    ...
    if (form.id) {
      await supabase.from('health_logs').update(payload).eq('id', form.id);
    } else {
  ...
  const handleDelete = async () => {
    ...
    await supabase.from('health_logs').delete().eq('id', deleteTarget.id);
